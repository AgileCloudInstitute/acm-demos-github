...
def createSequence(userName, groupName, keyPairName, operation):
  ...
  if operation == "awsfiles":
    ...
    resultsDict["AccessKeyId"] = AccessKeyId
...
def deleteSequence(userName, groupName, keyPairName, accessKeyID):
  ...
  print("Deleted access key with ID ", accessKeyID)
...
def malformedMessage():
  ...
  print("python3 acmDemoSetup.py destroy userName=validUserName groupName=validGroupName keyPairName=validKeyPairname accessKeyID=ValidAccessKeyIdThatWasCreatedByCreateCommand")
  quit()
...
def createAzureSequence(subscriptionId, subscriptionName, tenantId, appRegistrationName, clientSecretName, clientSecretEndDate, operation):
  ...
  addSubscriptionOwnerCommand='az deployment sub create --location WestUS --template-file '+templateName+' --parameters principalId='+spid
  getShellJsonResponse(addSubscriptionOwnerCommand)
  print("Done adding subscription owner role. ")

  # Assign AD Global Administrator
...
def generateKeysFile(keysDict):
  ...
  if 'AccessKeyId' in keysDict:
    line = "AWSAccessKeyId: "+keysDict["AccessKeyId"]
  else:
